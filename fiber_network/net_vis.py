...
"""
net_vis.py — render and animate VTKHDF beam/fiber networks via ParaView.
...
Typical use is to visualize e.g. HyperHDG plot.hxx (vtkhdf) output or
basis functions / eigenmodes as columns produced by netcoarse.py,
with the 6/7/8 components of the 'values' array interpreted as displacement
and 9/10/11 as rotation (Timoshenko DoFs).

Examples
--------
    net_vis.py field.vtkhdf -r 20 --view iso --warp-scale 1e3
    net_vis.py field.vtkhdf --color-by values:0 --warp-by none --arrows 0
    net_vis.py field.vtkhdf --frames 0.0,0.3,0.6 --frame-colors viridis
...
def net_vis(path, ops=(SolidColor("white")), bg="black", view="iso", resolution=(1000, 1000),
...
def net_vis_overlay(path, ops_frames, times, bg="black", view=None,
...
if __name__ == "__main__":
  ...
  if args.frames:
    ...
    net_vis_overlay(args.input, ops, times, bg=args.bg,
                   view=View(args.view), axis=args.axis, resolution=resolution,
                   output=args.output, show=args.show)
  else:
    ops = ops_factory(args.fg)

    net_vis(args.input, ops=ops, bg=args.bg, view=View(args.view), axis=args.axis,
